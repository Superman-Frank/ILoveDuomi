...
var GetAutoLoginInfo=function(){
	cordova.exec(
	function (data) {
		var url=data[actionData.OAPlugin.Login_SubmitAndRead.SysUrl].replace(/^http:\/\//i,'');
...
//    var _url="http://"+$("#txt_serverUrl").val();
	...
	if($("#chk_autoLogin").prop('checked')){
		//alert('自动登录！');
		jsonData+=","+actionData.OAPlugin.Login_SubmitAndRead.AutoLogin+":\"1\""+
...
//    var posturl=_url+"/ruvarOA/mobile/basehandle.ashx";

	//posturl+="?_t="+parseInt(Math.random()*10000000+1);
	/*var postform="<form  method=\"post\" name=\"form1\" id=\"form1\" action=\""+posturl+"\">";
	postform+="<input type=\"text\" id=\"account\" name=\"account\" value=\""+json[0][actionData.OAPlugin.Login_SubmitAndRead.LoginUser]+"\"/>";
	postform+="<input type=\"password\" id=\"pwd\" name=\"pwd\" value=\""+json[0][actionData.OAPlugin.Login_SubmitAndRead.LoginPwd]+"\"/>";
...
//    var param='action=action_login&dtype=1&token='+ v_token +'&account='+account+'&pwd='+pwd+'&_t='+parseInt(Math.random()*100000+1);
	var param='action=action_login&dtype=1&token='+ v_token +'&account='
	 + json[0][actionData.OAPlugin.Login_SubmitAndRead.LoginUser] + '&pwd='
	 + json[0][actionData.OAPlugin.Login_SubmitAndRead.LoginPwd] + '&goback_url='
	 +v_HomePage+'&_t='+parseInt(Math.random()*100000+1);

	console.log(param);

	if(v_XGURL!=null&&v_XGURL!=""){
		param='action=action_logoin&dtype=2&token='+ v_token +'&account='
		+ json[0][actionData.OAPlugin.Login_SubmitAndRead.LoginUser] + '&pwd='
		+ json[0][actionData.OAPlugin.Login_SubmitAndRead.LoginPwd] + '&goback_url='
        ...
        success: function (result) {
            ...
            if(result.status=="success"){
                ...
                if(v_XGURL=="")
                {
                    ...
                    //登录成功，跳转到home.html页面
           alert(_url+ "/RuvarOA/mobile/home.html?name="
                 +result.u_name                      //名字
                 ...
                 +"&u_split=1");
                    location.href = _url+ "/RuvarOA/mobile/home.html?name="
                    +result.u_name                      //名字
                    +"&u_eid="+result.u_eid             //档案id
                    +"&u_id="+result.u_id               //员工id
                    +"&u_bind="+result.u_bind           //是否已绑定员工自助账号
