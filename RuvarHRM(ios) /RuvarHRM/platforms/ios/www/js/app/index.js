...
//      cordova.exec(                        //调用java中的插件代码
//      function (data) {                    //成功的回调
//          wifiJson=data[actionData.HRMPlugin.GetWifi.wifiJson];
//      },
//      function (data) {                    //失败的回调
//          document.write(data.toString());
//      },
//      action.HRMPlugin.Name,               //插件的name （action.config中插件的name）
//      action.HRMPlugin.Action_WifiInfo,    //插件连接的url（action.类名.字符串）
//      []                                   //参数（不用携带）
//      );
//  };
...
var doLogin=function(){
     ...
     var _url="";
     var ss=$("#txt_serverUrl").val().replace(/^http:\/\//i,'');
     var geturl="http://"+ss+"/Config/appserver.xml?fresh=" + Math.random();
     doTip("路径请求中...", 0);
     $.ajax({
       type:"GET",
       url:geturl,
       ...
       success:function(xml){
            _url=$(xml).find('url').find('ios').text();
            doTip("路径请求成功！", 0);
            //获取登录相关
            // var _url="http://"+$("#txt_serverUrl").val().replace(/^http:\/\//i,'');
       var jsonData="[{"+actionData.HRMPlugin.Login_SubmitAndRead.SysUrl+":\""+$("#txt_serverUrl").val().replace(/^http:\/\//i,'')+"\""
     ...
     if(json[0][actionData.HRMPlugin.Login_SubmitAndRead.LoginUser]==null||json[0][actionData.HRMPlugin.Login_SubmitAndRead.LoginUser]==""
        ||json[0][actionData.HRMPlugin.Login_SubmitAndRead.LoginPwd]==null||json[0][actionData.HRMPlugin.Login_SubmitAndRead.LoginPwd]==""
         ...
         /* 网络访问*/
         //url
         var posturl=_url+serviceurl;
         subLogin(json[0][actionData.HRMPlugin.Login_SubmitAndRead.LoginUser],
                  json[0][actionData.HRMPlugin.Login_SubmitAndRead.LoginPwd],v_token,posturl);
         },
         error: function(XMLHttpRequest, textStatus, errorThrown) {
            alert(JSON.stringify(XMLHttpRequest))
            alert(textStatus)
            doTip("00服务器地址或网络错误！",5000);
            return false;
         }
         });
       };
           ...
           var param={"username":LoginUser,"password":LoginPwd,"deviceId":v_token};
           $.ajax({ //一个Ajax过程
                  ...
                  error: function (XMLHttpRequest, textStatus, errorThrown) {
                      ...
                      $("#login").removeAttr("disabled"); //登录按钮
                                                                               ...
                                                                               //    var param={"username":json[0][actionData.HRMPlugin.Login_SubmitAndRead.LoginUser]
                                                                               //    ,"password":json[0][actionData.HRMPlugin.Login_SubmitAndRead.LoginPwd],"deviceId":v_token};
                                                                               //    $.ajax({ //一个Ajax过程
                                                                               //        type: "post", //以post方式与后台沟通
                                                                               //        url: posturl, //与此ashx页面沟通
                                                                               //        contentType:"application/x-www-form-urlencoded; charset=UTF-8",
                                                                               ...
                                                                               //        SysUrl = data[actionData.HRMPlugin.SysUrl];             //预加载时，获取服务器地址到 SysUrl全局变量中
                                                                               //        console.log(SysUrl);
                                                                               //
                                                                               //    }, function(data) {
                                                                               //    }, action.HRMPlugin.Name, action.HRMPlugin.Action_SysUrl, [{}]);
                                                                               //};
