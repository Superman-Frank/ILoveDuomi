...
- (void)XG_GetToken:(CDVInvokedUrlCommand *)command
{
	...
	NSLog(@"HRMPlugin=============>XG_GetToken:%@\n",app.deviceID);
	CDVPluginResult* pluginResult=nil;
	NSMutableDictionary* mdic=[NSMutableDictionary dictionaryWithCapacity:2];

	[mdic setObject:@"0" forKey:@"ret_code"];
	if(app.deviceID !=nil){
	[mdic setObject:app.deviceID forKey:@"token"];
	}
	else{
		[mdic setObject:@"notokensstring" forKey:@"token"];
	}
...
- (void)RV_Login_Read:(CDVInvokedUrlCommand*)command
{
	...
	NSString* SysUrl=[defaults valueForKey:@"_sysUrl"];
 	NSString * autoLogin =[defaults valueForKey:@"_autologin"];
	NSString * LoginPwd=[defaults valueForKey:@"_loginpwd"];
	NSString * LoginUser=[defaults valueForKey:@"_loginUser"];
	NSString* SavePwd=[defaults valueForKey:@"_savepwd"];
